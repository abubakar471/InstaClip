    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            if (checkQuotaStatus) {
                ...
                const response = await fetch(`${process.env.NEXT_PUBLIC_FLASK_API_URL}/video/create-voice-over`, {
                    method: "POST",
                    body: formData,
                });
        ...
        <div className={cn("", className)}>
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
                <DialogTrigger asChild>
                    <Button variant="outline" className="grow mt-2 bg-[#36339E] !text-white py-1 px-3 rounded hover:bg-blue-600 flex items-center justify-start gap-x-2 border-none text-sm font-light w-full">
                        <MdKeyboardVoice />
                ...
                <DialogContent className="sm:max-w-[425px] md:max-w-[500px] min-h-[85vh] bg-black/95 border border-white/10 backdrop-blur-sm text-white">
                    ...
                    <form onSubmit={handleUpdate} className='mt-6'>
                        <div className='flex items-center gap-x-2 mb-4'>
                            ...
                            <h4 className='text-neutral-400 text-xs'>Write Your Voice-over Script</h4>
                        </div>
                        <textarea value={voiceScript} onChange={e => setVoiceScript(e.target.value)} className='w-full bg-[#fff]/10 text-white rounded-md px-4 py-3 text-sm focus:ring-0 outline-none min-h-[10vh]' placeholder='Your voice-over script'>
                        </textarea>

                        <div className='w-full flex items-center gap-x-2'>
                            <Select onValueChange={(value) => setAudioVolumeRatio(Number(value))} className="">
                            ...
                            <Select onValueChange={(value) => setVideoVolumeRatio(Number(value))} className="">
                                <SelectTrigger className="w-1/2 text-neutral-300 border-t-0 border-r-0 border-l-0 border-b-2 border-gray-500/40 !bg-[#333]/50 !drop-shadow-none py-4 px-4 rounded-md">
                                    <div className='flex items-center gap-x-2 py-2'>
                                        <AiOutlineAudio />
                        ...
                        <div className='w-full flex items-center gap-x-2 mt-2'>
                            <Select onValueChange={(value) => setVoiceType(value)} className="">
                                <SelectTrigger className="w-1/2 text-neutral-300 border-t-0 border-r-0 border-l-0 border-b-2 border-gray-500/40 !bg-[#333]/50 !drop-shadow-none py-4 px-4 rounded-md">
                                    <div className='flex items-center gap-x-2 py-2'>
                                        <IoMaleFemale />
                        ...
                        >
                            {isUpdating ? <AiOutlineLoading3Quarters className="text-white animate-spin" /> : "Create Voice-over"}
                        </Button>
                    </form>
                    ...
                    <DialogFooter>
                        {
                            (isCreated && !isUpdating && newVideo) && (
                                <div key={videoRenderKey} className='w-full'>
                                    <Video className="w-full h-[400px] rounded-3xl mt-4">
                                        <source src={`${newVideo?.location}`} type='video/mp4' className='' />
                                    </Video>
                                    <SaveEditClip
