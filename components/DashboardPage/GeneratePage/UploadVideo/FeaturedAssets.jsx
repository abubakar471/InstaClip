...
import { assets } from '@/data/featured_assets'
...
const FeaturedAssets = () => {
    ...
    return (
        <div className='grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-4 mt-6'>
            {
                (assets?.length > 0 && isLoaded) ? assets?.map((item, index) => (
                    ...
                    Array.from(new Array(3))?.map((item, i) => (
                        <div key={i} className='h-[200px] bg-[#101D28] rounded-2xl'>
