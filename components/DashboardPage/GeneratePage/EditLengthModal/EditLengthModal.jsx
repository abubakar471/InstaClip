...
const EditLengthModal = ({ clip, clips, setClips, className }) => {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_FLASK_API_URL}/video/trim-video`, {
                method: "POST",
                body: bodyData,
            });
            ...
            if (data?.success) {
                ...
                const response = await axios.post(`${process.env.NEXT_PUBLIC_NODE_API_URL}/assets/trim-video`, {
                    ...
                    <form onSubmit={handleUpdate} className='-mt-6'>
                        ...
                        <div className='w-full flex items-center gap-x-2 gap-y-2 flex-col lg:flex-row mt-4'>
                            <div className='flex flex-col gap-y-2 w-full lg:w-1/2'>
                                <label className='text-neutral-100 text-xs'>Start {"(s)"}</label>
                                <input
                                    type="number"
                                    placeholder="0"
                        ...
                        >
                            {isUpdating ? <AiOutlineLoading3Quarters className="text-white animate-spin" /> : "Update"}
                        </Button>
                    </form>
